import React from 'react';
import type { ButtonHTMLAttributes } from 'react';
import clsx from 'clsx'; // Import clsx

interface ButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'danger' | 'ghost';
  isLoading?: boolean;
  fullWidth?: boolean;
}

const Button: React.FC<ButtonProps> = ({
  children,
  variant = 'primary',
  isLoading = false,
  fullWidth = false,
  className: propClassName, // Rename to avoid conflict with `className` in clsx
  ...props
}) => {
  const baseStyle = "inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2";
  const fullWidthStyle = fullWidth ? "w-full" : "";

  let variantStyle = '';
  switch (variant) {
    case 'secondary':
      variantStyle = 'text-indigo-700 bg-indigo-100 hover:bg-indigo-200 focus:ring-indigo-500';
      break;
    case 'danger':
      variantStyle = 'text-white bg-red-600 hover:bg-red-700 focus:ring-red-500';
      break;
    case 'ghost':
      variantStyle = 'text-gray-700 bg-transparent hover:bg-gray-100 focus:ring-indigo-500';
      break;
    case 'primary':
    default:
      variantStyle = 'text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500';
      break;
  }

  const disabledStyle = props.disabled || isLoading ? 'opacity-50 cursor-not-allowed' : '';

  // Use clsx to combine all class names
  const mergedClassName = clsx(
    baseStyle,
    variantStyle,
    fullWidthStyle,
    disabledStyle,
    propClassName // Prop-provided className comes last to override
  );

  return (
    <button
      className={mergedClassName}
      disabled={props.disabled || isLoading}
      {...props}
    >
      {isLoading ? (
        <>
          <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
          Processing...
        </>
      ) : (
        children
      )}
    </button>
  );
};

export default Button;
